...
function [Xguess,shiftMap,dispMap] = recon_fRL_GPU_single( psfs,psfs_sim,psfs_diff,projs,vortex_index,reconOpts,Xguess)
% INPUT
...
if DAO
    ...
    if ~isfield(DAOOpts,'sidelobe');DAOOpts.sidelobe=0;end
    if ~isfield(DAOOpts,'sidelobez');DAOOpts.sidelobez=0;end
...
for iterNow = 1:maxIter
    ...
    for angleNow = 1:angleNum
        ...
        errorBack = backwardProj_fRL_GPU(largepsf,HXguessFFT,projNow) + errorBack;
        ...
        %%%finish
        clear errorBack errorBack_new;%To avoid out of GPU memory
    ...
    for diffNow = 1 : diffNum
        largepsf((proj_r-psf_r)/2+1:(proj_r+psf_r)/2,(proj_c-psf_c)/2+1:(proj_c+psf_c)/2,:) = gpuArray(squeeze(psfs_sim(:,:,:,diffNow)));
        HXguessFFT = forwardProj_fRL_GPU(largepsf,Xguess);
        HXguess = real(ifftn(HXguessFFT));
        HXguess = HXguess./sum(HXguess,'all');
        vortex_LF = squeeze(projs(:,:,vortex_index(diffNow)));
        vortex_LF = vortex_LF./sum(vortex_index,'all');
        diff_LF = HXguess - 0.6 * vortex_LF;
