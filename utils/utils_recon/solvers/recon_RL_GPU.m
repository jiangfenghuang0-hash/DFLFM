...
if DAO
    ...
    if ~isfield(DAOOpts,'patchN');DAOOpts.patchN=7;end
    ...
    if ~isfield(DAOOpts,'sidelobe');DAOOpts.sidelobe=10;end
    if ~isfield(DAOOpts,'sidelobez');DAOOpts.sidelobez=5;end
...
for iterNow = 1:maxIter
    ...
    if DAO == 1 && iterNow > maxIter_init
        ...
        dispMap = dispMap - dispMap(:,:,:,11);
        shiftMap = shiftMap - shiftMap(:,:,:,11);
        if dispOn
            fprintf(' done\n');
        end
    end
    ...
    for angleNow = 1:  angleNum
        ...
        errorEM(isnan(errorEM)) = 0;
        errorEM(isinf(errorEM)) = 0;
        errorEM(errorEM < 0) = 0;
        if ~exist('errorBack','var'); errorBack = gpuArray.zeros(proj_r,proj_c,psf_s,'single'); end
        errorBack = backwardProj_RL_GPU(gpuArray(squeeze(psf_t(:,:,:,angleNow))),errorEM)./ htf(:,:,:,angleNow) + errorBack;
%          errorBack = backwardProj_RL_GPU(gpuArray(squeeze(psf_t(:,:,:,angleNow))),errorEM)+errorBack;

        %%%update Volume
        if angleNow == 11
            Xguess = Xguess.*errorBack.*upWeight/2+(1-upWeight/2).*Xguess;
        else
...
%     for angleNow = 1:  angleNum
%         HXguess_end(:,:,angleNow) = forwardProj_RL_GPU(gpuArray(squeeze(psfs(:,:,:,angleNow))),Xguess);
%     end

%     %%% Save
%     stackTempForSave = gather(Xguess);
%  
%     stackTempForSave = stackTempForSave./max(stackTempForSave(:)) * 65535;
% 
%     if DAO == 1
%         filename = 'AO';
%     else
%         filename = 'NO AO';
%     end
%     saveastiff_overwrite(stackTempForSave, [savepath, ['\',filename, '_Xguess_iter'],num2str(iterNow),'.tif'],matSaveOn,tifSaveOn);
end