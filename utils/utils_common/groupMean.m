% This function group-mean-projects an input stack according to given
...
%%ELiiiiiii, 20210819
function projResult = groupMean(inputStack, groupSize, cropFlag, projDim)
%% defaults
if nargin < 4
    projDim = 3;
end
...
for projCount = 1:projResultSize3
    projResult(:,:,projCount) = squeeze(mean(inputStack(:,:,(projCount-1)*groupSize+1:min(projCount*groupSize,size3)),projDim));
end